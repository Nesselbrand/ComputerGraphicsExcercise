...
uniform sampler2D checkerboardTexture;  // from Basic3.js TEXTURE0
uniform sampler2D cobblestoneTexture;   // from Basic3.js TEXTURE1
...
void main(void) {
	...
	if(cobblestone) {
		...
		//				dummy line.
// Plane should be covered once → scale by planeSize
vec2 tiledTC = texCoordV / planeSize;

vec3 normalTex = texture2D(cobblestoneTexture, tiledTC).rgb;

// Map from [0,1] → [-1,1]
n = normalize(normalTex * 2.0 - 1.0);

// Use constant color for cobblestone
color = vec3(0.5, 0.5, 0.5);
	} else {
		n = normal;
		...
		color = texture2D(checkerboardTexture, texCoordV).rgb;
